...
<head>
<meta charset="UTF-8">
<link rel="stylesheet" href="/resources/css/press/list.css">
...
<table>
        <div class="reviewTot">
              <div class="reviewList">
                          <c:forEach items="${pressBoardList}" var="pressboard">
  <div class="list li l20">
    <div id="reviewTitle title t20">
      <h5>
        <a href="/press/detail?${pressboard.no}">${pressboard.title}</a>
      </h5>
    </div>
    <div class="reviewDate date d20">
      <h5>${pressboard.enroll_date}</h5>
    </div>
  </div>
</c:forEach>
          ...
          <div class="listBox">
              <div class="listButton">
                  ...
                  <div class="listnum list1"><a href="/press/list?page=1"><h5>1</h5></a></div>
                  <div class="listnum list2"><a href="/press/list?page=2"><h5>2</h5></a></div>
                  <div class="listnum list3"><a href="/press/list?page=3"><h5>3</h5></a></div>
                  <div class="listnum list4"><a href="/press/list?page=4"><h5>4</h5></a></div>
                  <div class="listnum list5"><a href="/press/list?page=5"><h5>5</h5></a></div>
...
<c:if test="${memberLog != null}">
      <div id="recruit-text">
        <button id="writeBtn" onclick="location.href='/press/write'">
          ...
          <span class="now">작성!</span>
          <span class="play">게시글</span>
        </button>
      </div>
    </c:if>
          </div>
      ...
      <c:if test="${paginationVo.currentPage > 1}">
        <%-- <a href="/press/list?page=${paginationVo.currentPage-1}"><</a> --%>
      </c:if>
      <c:forEach var="i" begin="${paginationVo.startPage}" end="${paginationVo.endPage}" step="1">
        ...
        <c:if test="${paginationVo.currentPage != i}">
          <%-- <a href="/press/list?page=${i}">${i}</a> --%>
        </c:if>
      </c:forEach>
      <c:if test="${paginationVo.currentPage < paginationVo.maxPage}">
        <%-- <a href="/press/list?page=${paiginationVo.currentPage+1}">></a> --%>
      </c:if>
    </div>
...
<script>
  ...
  tbody.addEventListener('click', (event) => {
    ...
    /* window.location.href = "/press/detail"+"?" + no; */
    window.location.href = `/press/detail?${no}`;
  });
...
<div><a href="${root}/press/write">P</a></div>    
</html>